...
[Download v0.2.2](https://github.com/DLYZZT/pi-desktop/releases/tag/v0.2.2) · [Screenshots](#screenshots) · [Features](#features) · [Quick start](#quick-start) · [Architecture](#architecture) · [Changelog](https://github.com/DLYZZT/pi-desktop/releases) · [Roadmap](#roadmap)
...
- Automatically generate a short background title from a new session's first meaningful message; the setting is optional and manual names always win
...
Managed background processes in v0.2.2 support macOS, Linux, and Windows 11 x64. Windows ARM64, Windows Server, and 32-bit Windows are not supported.
...
The latest stable version is [v0.2.2](https://github.com/DLYZZT/pi-desktop/releases/tag/v0.2.2), with builds for macOS Apple Silicon and Intel, Windows x64, and Linux x64.

Pi Agent Desktop v0.2.2 bundles the Pi Coding Agent 0.85.0 runtime. Regular users do not need to install the Pi CLI, Pi Coding Agent, Node.js, or npm just to use the Agent. When a Skill, Plugin, or Agent script needs additional developer tools, the application first reuses healthy system installations and can install private runtimes after explicit user confirmation.
