...
function isIssuesDisabled(stderr: string): boolean {
	return /has disabled issues/i.test(stderr);
}
...
export default function (pi: ExtensionAPI): void {
	pi.on("session_start", async (_event, ctx) => {
		...
		if (!resolvedRepo.ok) {
			// Home / non-git / non-GitHub cwd: feature does not apply. Do not toast.
			return;
		}
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				...
				if (result.code !== 0) {
					// Repo has Issues off: feature does not apply. Do not toast.
					if (!loadErrorShown && !isIssuesDisabled(result.stderr)) {
						loadErrorShown = true;
